...
**Live Translator** is a browser-based application that enables seamless real-time translation of spoken audio. Using the Gemini Live API, it captures audio from your microphone and/or screen, translates it between selected language pairs, and speaks the translation back to you¡ªall in real-time with minimal latency.

### Key Features

- **Real-time bidirectional translation** ¨C Automatically detects the spoken language and outputs translation in the other language
- **Multiple audio sources** ¨C Capture from microphone, app/tab audio, or both simultaneously
- **8+ languages supported** ¨C English, Mandarin Chinese, Bengali, Japanese, Korean, Spanish, French, German, Arabic, Hindi, Portuguese, and more
- **Multiple voices** ¨C Choose from 8 distinct AI voices (Zephyr, Puck, Charon, Kore, Fenrir, Aoede, Leda, Orus)
- **Customizable system prompts** ¨C Tailor the translation behavior to your needs
- **Session management** ¨C Track session duration and conversation history
- **Pop-out window** ¨C Use Picture-in-Picture mode for multitasking
- **Chat logging** ¨C View and export conversation history
- **Mobile-responsive UI** ¨C Sidebar-based settings work seamlessly on mobile and desktop
- **Zero data server** ¨C API key and preferences stored only in your browser

## Project Structure

```
live-translator/
©À©¤©¤ index.html           # Main HTML - header, sidebar, chat, modal sheets
©À©¤©¤ css/
©¦   ©¸©¤©¤ style.css       # Responsive mobile-first styling
©¸©¤©¤ js/
    ©À©¤©¤ app.js          # UI glue, lifecycle, preferences, DOM management
    ©À©¤©¤ audio.js        # Audio capture & PCM processing (mic, display, TTS playback)
...
- **Features**:
  ...
  - State machine (idle ¡ú connecting ¡ú connected ¡ú error ¡ú reconnecting)
...
- No external dependencies¡ªuses native Web Audio, WebSocket, and Fetch APIs
